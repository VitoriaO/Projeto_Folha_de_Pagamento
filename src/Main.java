...
public class Main{
	...
	public static void copiar(Empregado emporiginal, Empregado emp){
		...
		emp.dia = emporiginal.dia;
		emp.d_venda = emporiginal.d_venda;
	...
	public static void novaAgenda(Scanner entrada, Agenda agenda){
		...
		if(aux.equals("mensal") || aux.equals("Mensal")){
			...
			dia = entrada.nextInt();
			agenda.d_mes[dia-1] = true;
		}
		else{
			System.out.println("Em que dia da semana o funcionário deverá ser pago? (1 - segunda, 2 - terça ...)");
			dia = entrada.nextInt();
			agenda.d_semana[dia-1] = true;
		}
	}
	
	public static void imprimirFunc(Empregado[] emps){
		int i;
		for(i = 0; i < 20; i++){
			if(emps[i] != null){
				System.out.println("Nome: " + emps[i].nome +
						"\nId: " + emps[i].id +
						"\nEndereço: " + emps[i].endereco +
						"\nTipo: " + emps[i].tipo +
						"\nSalário: " + emps[i].salario);
				if(emps[i].tipo.equals("comissionado") || emps[i].tipo.equals("Comissionado")){
					System.out.println("\nComissão: " + emps[i].comissao);
				}
				if(emps[i].sindicato == true){
					System.out.println("\nEmpregado faz parte do sindicato\n");
				}
				else{
					System.out.println("\nEmpregado não faz parte do sindicato\n");
				}
			}
			else{
				break;
	...
	public static void main(String[] args){
		...
		do{
			...
			System.out.println("10. Imprimir funcionários cadastrados");
			System.out.println("11. Desfazer");
			System.out.println("12. Refazer");
